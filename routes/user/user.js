...
router.get("/user-subscription", [appAuth, auth], async (req, res) => {
  ...
  return res.send(
    createResObject(
      true,
      {
        cardsLeft: skipPayment ? "Unlimited" : cardsLeft,
        subId: skipPayment ? "sub_high" : subId,
      },
      stringConstants.FETCH_SUCESSFUL
    )
  );
...
router.get("/cancel-subscription", [appAuth, auth], async (req, res) => {
  ...
  if (subId) {
    ...
    return res.send(createResObject(true, { user }, "Subscription Cancelled"));
  }
...
router.get("/authorize", [appAuth, auth], async (req, res) => {
  // const user = await User.findById(req.user._id);
  return res.send(createResObject(true, {}, "Authorized User"));
});

/**
 * Generate OTP
 */
router.post("/generate-otp", [appAuth], async (req, res) => {
  ...
  if (success) {
    return res.send(createResObject(true, stringConstants.OTP_GENERATED));
  }
 ...
 * Verify OTP
 */
router.post("/verify-otp", [appAuth, valVerifyOtp], async (req, res) => {
  ...
  if (dbOtp) {
    ...
    return res
      .status(500)
      .send(
        createResObject(false, {}, stringConstants.NO_OTP, errorObjects.NO_OTP)
      );
  }

  return res.send(createResObject(true, stringConstants.VERIFY_OTP));
});
...
router.post(
  ...
  async (req, res) => {
    ...
    return res.send(
      createResObject(true, stringConstants.PASSWORD_UPDATED_SUCCESSFULLY)
    );
  }
);

/**
 * Get Stripe account token
 */
router.post("/stripe-auth", [appAuth, auth], async (req, res) => {
  ...
  try {
    ...
    let isStripeAttached = await StripeConnect.findOne({
      user: mongoose.Types.ObjectId(userId),
    });
    let stripeAccount;
    if (isStripeAttached) {
      stripeAccount = await StripeConnect.findByIdAndUpdate(
        isStripeAttached.id,
        {
          $set: {
      ...
      stripeAccount = await StripeConnect.create({
        accessToken: response.access_token,
        livemode: response.livemode,
        refreshToken: response.refresh_token,
        tokenType: response.token_type,
    ...
    return res.send(
      createResObject(
        true,
        stripeAccount,
        stringConstants.STRIPE_ACCOUNT_CONNECT_SUCCESSFULLY
      )
    );
  } catch (e) {
